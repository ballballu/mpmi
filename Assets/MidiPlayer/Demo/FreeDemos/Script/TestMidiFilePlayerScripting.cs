...
namespace DemoMPTK
{
    public class TestMidiFilePlayerScripting : MonoBehaviour
    {
        ...
        public void StartPlay(string name)
        {
            ...
            Debug.Log($"Start Play Midi '{name}'  Duration: {midiFilePlayer.MPTK_DurationMS / 1000f:F2} seconds  Load time: {midiFilePlayer.MPTK_MidiLoaded.MPTK_LoadTime:F2} milliseconds");
        }
        ...
        private void ChannelDisplay()
        {
            ...
            for (int channel = 0; channel < midiFilePlayer.MPTK_ChannelCount(); channel++)
            {
                ...
                // Slider to change the preset on this channel. Forced bank from the inspector.
                int forcePreset = (int)GUILayout.HorizontalSlider(midiFilePlayer.MPTK_ChannelPresetGetIndex(channel),
                                                    -1f, 127f, myStyle.SliderBar, myStyle.SliderThumb, GUILayout.Width(100));
                if (forcePreset != midiFilePlayer.MPTK_ChannelPresetGetIndex(channel))
                {
                    // Force a preset and a bank whatever the MIDI events from the MIDI file.
                    // set forcePreset to -1 to restaure to the last preset and bank value known from the MIDI file.
