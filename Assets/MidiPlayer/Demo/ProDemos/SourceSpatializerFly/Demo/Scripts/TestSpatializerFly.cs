...
namespace DemoMPTK
{
    ...
    public class TestSpatializerFly : MonoBehaviour
    {
        ...
        private void Start()
        {
            ...
            //Debug.Log($"Start TestSpatializerFly {midiSpatializer.MPTK_DedicatedChannel}");
            ...
            else
            {
                ...
                if (!midiSpatializer.OnEventNotesMidi.HasEvent())
                {
            ...
            if (!midiSpatializer.OnEventStartPlayMidi.HasEvent())
            {
                // Set event by script
        ...
        public void ArrangeByChannel()
        {
            ...
            for (int iChannel = 0; iChannel < 16; iChannel++)
            {
                ...
                // Debug.Log($"ArrangeByChannel {mfp.MPTK_DedicatedChannel} {lineNumber} { ((float)lineNumber) / 4f}");
        ...
        public void ArrangeByTrack()
        {
            ...
            for (int iTrack = 0; iTrack < CountTrack; iTrack++)
            {
                ...
                // Debug.Log($"ArrangeByTrack {mfp.MPTK_DedicatedChannel} {lineNumber} ");
        ...
        /// Call only from the Midi reader (MPTK_DedicatedChannel == -1)
        /// </summary>
            ...
            foreach (MPTKEvent midievent in events)
            {
                switch (midievent.Command)
                {
                    ...
                    case MPTKCommand.PatchChange:
                        ...
                        {
                            // If the patch is changed, the position of the sphere changed also
        ...
        void Update()
        {
            if (midiSpatializer != null)
            {
                ...
                else
                {
                    ...
                    // If mode move random and synth is avtive
                    ...
                    if (ScaleSynth > 0f)
                    {
                        ...
                        //if (midiSpatializer.MPTK_DedicatedChannel == 0) Debug.Log(transform.localScale);
                    }
                    ...
                    if (MidiMaster.MPTK_ModeSpatializer == MidiSynth.ModeSpatializer.Channel)
                    {
                        if (midiSpatializer.MPTK_SpatialSynthIndex == 9)
                            presetName = "Drums"; // Default preset name is "Standard", not really useful.
                        presetName = midiSpatializer.MPTK_ChannelPresetGetName(midiSpatializer.MPTK_SpatialSynthIndex);
                    }
                    else
                        presetName = midiSpatializer.MPTK_InstrumentPlayed;

                    //Debug.Log($"{midiSpatializer.MPTK_DedicatedChannel} {presetName}");
                    textPlayer.text = presetName;
